...
class SpeechToTextBenchmark:
    ...
    def _setup_providers(self):
        ...
        # OpenAI providers
        try:
            self.providers["OpenAI-GPT4o-Transcribe"] = OpenAIWhisperProvider("gpt-4o-transcribe")
    ...
    async def benchmark_provider(self, provider_name: str, provider: SpeechToTextProvider) -> BenchmarkResult:
    ...
    async def run_benchmark(self, num_runs: int = 3) -> Dict[str, List[BenchmarkResult]]:
    ...
    def analyze_results(self, results: Dict[str, List[BenchmarkResult]]) -> None:
        ...
        for provider_name, provider_results in results.items():
            ...
            if successful_runs:
                ...
                avg_transcript_length = sum(len(t.split()) for t in transcripts) / len(transcripts)
                ...
                stats[provider_name] = {
                    ...
                    "sample_transcript": successful_runs[0].transcript[:100] + "..." if successful_runs[0].transcript else "No transcript"
                }
            else:
                stats[provider_name] = {
                    "success_rate": 0.0,
        ...
        print(f"{'Provider':<25} {'Success Rate':<12} {'Avg Latency':<12} {'Min Latency':<12} {'Max Latency':<12} {'Avg Words':<10}")
        print("-" * 100)
        
        for provider_name, stat in sorted_stats:
            if stat["success_rate"] > 0:
                print(f"{provider_name:<25} {stat['success_rate']:<12.1%} {stat['avg_latency']:<12.2f}s {stat['min_latency']:<12.2f}s {stat['max_latency']:<12.2f}s {stat['avg_transcript_length']:<10.0f}")
            else:
                print(f"{provider_name:<25} {'FAILED':<12} {'N/A':<12} {'N/A':<12} {'N/A':<12} {'N/A':<10}")
        ...
        fast_providers = [name for name, stat in sorted_stats if stat["success_rate"] > 0 and stat["avg_latency"] < 2.0]
        reliable_providers = [name for name, stat in sorted_stats if stat["success_rate"] >= 0.8]
        
        if fast_providers:
            print(f"Fastest providers (< 2s): {', '.join(fast_providers)}")
        
        if reliable_providers:
            print(f"Most reliable providers (≥80% success): {', '.join(reliable_providers)}")
        
        if sorted_stats:
            fastest = sorted_stats[0]
            if fastest[1]["success_rate"] > 0:
                print(f"Recommended for speed: {fastest[0]} ({fastest[1]['avg_latency']:.2f}s average)")
